...
# ════════════════════════════════════════════════════════════════
# 2. Dumbbell Chart — 전후 성적 비교
#    ※ '전후 비교'는 두 시점을 점으로 찍고 선으로 이어 변화량(기울기)을
#      직접 보여주는 덤벨차트가 막대그래프보다 더 직관적으로 정보를 전달합니다.
# ════════════════════════════════════════════════════════════════
elif selected == "dumbbell":
    ai_yes = df[df["uses_ai"] == "Yes"]
    ai_no = df[df["uses_ai"] == "No"]

    groups = [
        {"label": f"AI 미사용 (n={len(ai_no)})", "before": ai_no["grades_before_ai"].mean(),
         "after": ai_no["grades_after_ai"].mean(), "color": COLOR_NO},
        {"label": f"AI 사용 (n={len(ai_yes)})", "before": ai_yes["grades_before_ai"].mean(),
         "after": ai_yes["grades_after_ai"].mean(), "color": COLOR_YES},
    ]
    ...
    for g in groups:
        fig.add_trace(go.Scatter(
            ...
            mode="lines", line=dict(color=g["color"], width=5),
            showlegend=False,
        ))
    fig.add_trace(go.Scatter(
        x=[g["before"] for g in groups], y=[g["label"] for g in groups],
        mode="markers+text", name="AI 도입 전",
        marker=dict(color="#B7B7B7", size=16),
        text=[f'{g["before"]:.1f}' for g in groups], textposition="top center",
    ))
    fig.add_trace(go.Scatter(
        x=[g["after"] for g in groups], y=[g["label"] for g in groups],
        mode="markers+text", name="AI 도입 후",
        marker=dict(color=[g["color"] for g in groups], size=18),
        text=[f'{g["after"]:.1f}' for g in groups], textposition="bottom center",
    ))
    fig.update_layout(
        title={"text": "AI 사용 전후 평균 성적 변화 (덤벨차트)", "x": 0.5},
        xaxis_title="평균 성적 (100점 만점)",
        yaxis_title="",
    )
    st.plotly_chart(fig, use_container_width=True)
    ...
    st.info(
        f"AI를 사용한 학생들의 평균 성적은 **+{avg_change_yes:.2f}점** 향상되었습니다(선의 기울기 참고). "
...
# ════════════════════════════════════════════════════════════════
# 5. Scatter Plot — 공부 시간 vs 성적 향상 (추세선 포함)
#    ※ 3구간으로 나눈 막대그래프는 임의의 구간 경계 때문에 실제보다
#      뚜렷한 차이가 있는 것처럼 보일 수 있습니다. 원본 데이터를 그대로
#      점으로 찍고 추세선을 더하면 실제 관계(거의 없음)를 왜곡 없이 보여줍니다.
# ════════════════════════════════════════════════════════════════
elif selected == "scatter":
    ai_df = df[df["uses_ai"] == "Yes"].copy()
    corr_val = ai_df["study_hours_per_day"].corr(ai_df["grade_change"])

    # numpy로 직접 선형 추세선 계산 (외부 패키지 의존성 없이)
    z = np.polyfit(ai_df["study_hours_per_day"], ai_df["grade_change"], 1)
    trend_x = np.linspace(ai_df["study_hours_per_day"].min(), ai_df["study_hours_per_day"].max(), 50)
    trend_y = np.polyval(z, trend_x)

    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=ai_df["study_hours_per_day"], y=ai_df["grade_change"],
        mode="markers", name="학생",
        marker=dict(color=COLOR_YES, size=9, opacity=0.7),
    ))
    fig.add_trace(go.Scatter(
        x=trend_x, y=trend_y, mode="lines", name="추세선",
        line=dict(color="#2C4870", width=3, dash="dash"),
    ))
    fig.update_layout(
        title={"text": f"공부 시간 vs 성적 향상 (산점도, r = {corr_val:.2f})", "x": 0.5},
    ...
    st.info(
        f"상관계수는 **r = {corr_val:.2f}**로 0에 매우 가까워, 공부 시간과 성적 향상 사이에는 "
        "뚜렷한 선형 관계가 보이지 않습니다(추세선이 거의 평평함). "
        "즉 단순히 공부 시간을 늘리는 것보다, **AI를 얼마나 효율적으로 활용하는지가 "
        "성적 향상에 더 중요한 요인**일 수 있음을 시사합니다."
    )

# ════════════════════════════════════════════════════════════════
# 6. Violin Plot — 교육 수준별 성적 향상 분포 (AI 사용 학생 대상)
#    ※ AI 미사용 그룹은 grade_change가 전원 정확히 0점으로 값의 편차가
#      전혀 없어, 그래프로 그려도 보여줄 분포 정보가 없습니다(모든 점이
#      한 줄로 겹침). 그래서 이 그래프는 실제 분포가 존재하는 AI 사용
#      학생만 다루고, 미사용 그룹은 아래 안내 문구로 별도 설명합니다.
# ════════════════════════════════════════════════════════════════
elif selected == "violin":
    yes_df = df[df["uses_ai"] == "Yes"].copy()
    yes_df["edu_label"] = yes_df["education_level"].map({"school": "중·고등학교", "college": "대학교"})
    ...
    fig = px.violin(
        ...
        box=True,
        points="all",
        category_orders={"edu_label": ["중·고등학교", "대학교"]},
        color_discrete_sequence=["#7B9FD1", "#2C4870"],
    )
    fig.update_traces(marker=dict(opacity=0.6, size=6))
    fig.update_layout(
        title={"text": "교육 수준별 성적 향상 분포 (바이올린 플롯, AI 사용 학생 대상)", "x": 0.5},
        xaxis_title="",
        yaxis_title="성적 향상도 (점)",
        showlegend=False,
    )
    ...
    for edu in ["school", "college"]:
        sub_ai = df[(df["education_level"] == edu) & (df["uses_ai"] == "Yes")]
        m = sub_ai["grade_change"].mean()
        st.info(f"**{edu_labels[edu]}**의 AI 사용 학생 평균 성적 향상: **+{m:.2f}점** (n={len(sub_ai)})")

    st.info(
        "💡 **AI 미사용 학생(60명)은 이 그래프에 포함하지 않았습니다.** 이 학생들은 성적 변화가 "
        "전원 정확히 0점으로 값의 편차가 전혀 없어, 분포 모양을 보여주는 바이올린 플롯으로는 "
        "표현할 정보가 없기 때문입니다(모든 점이 0에서 겹쳐 하나의 선처럼 보임). "
        "대신 위 안내처럼 숫자로 명시했습니다: **AI 미사용 학생의 평균 성적 변화는 0.00점**입니다."
    )
