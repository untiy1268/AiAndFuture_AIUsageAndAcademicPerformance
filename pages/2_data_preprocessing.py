...
if os.path.exists(csv_filename):
    ...
    total_removed_outliers = 0 # 전체 제거된 이상치 카운트 변수
    ...
    for col in [c for c in NUMERIC_TARGET_COLS if c in outlier_removed_df.columns]:
        ...
        total_removed_outliers += n_outliers
    ...
    # Tab 3: 이상치 분석 및 직관적인 전후 비교 설명 추가
    with tab3:
        ...
        with col_graph1:
            ...
            # 이상치가 없을 때 사용자가 안심하고 이해할 수 있도록 명확한 텍스트 알림창 추가
            if total_removed_outliers == 0:
                st.success("💡 **분석 결과 안내**\n\n현재 업로드된 데이터셋은 모든 수치형 변수가 IQR 기준 정상 경계선 내에 예쁘게 모여 있는 **'이상치(Outlier)가 전혀 없는 깨끗한 데이터셋'**입니다. 제거된 행이 없으므로 전처리 전과 후의 분포 결과가 완전히 같습니다.")
            else:
        ...
        with col_graph2:
            ...
            if available_num_cols:
                ...
                if total_removed_outliers == 0:
                    st.info(f"ℹ️ 탐지된 이상치 개수가 0개이므로 두 박스의 형태와 위치가 동일한 것이 정상입니다.")
            else:
