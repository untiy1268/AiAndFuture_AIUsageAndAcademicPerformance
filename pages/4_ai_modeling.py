...
with col_fig1:
    ...
    fig1.update_layout(title="실제 vs 예측 성적 비교", xaxis_title="실제 성적", yaxis_title="예측 성적", height=380, margin=dict(l=40, r=20, t=40, b=40))
    st.plotly_chart(fig1, use_container_width=True)

with col_fig2:
    # Random Forest 변수 중요도 Plotly 그래프
    importances = rf_model.feature_importances_
    feat_df = pd.DataFrame({"feature": X.columns, "importance": importances}).sort_values("importance", ascending=True)
    
    fig2 = go.Figure(go.Bar(x=feat_df["importance"], y=feat_df["feature"], orientation="h", marker=dict(color='#E8714C')))
    fig2.update_layout(title="Random Forest 변수 중요도", xaxis_title="Importance", yaxis_title="", height=380, margin=dict(l=40, r=20, t=40, b=40))
