...
@st.cache_data
def load_datasets():
    # 데이터셋 1: 전처리 전 원본 스타일 데이터 가정
    # (실제 데이터에 'uses_ai'가 없을 경우 파생 변수로 생성)
    if os.path.exists("students_ai_usage.csv"):
        ...
        # 대체 데이터가 존재할 경우 이를 기반으로 원본 형태 재현
    ...
    # 문자열 결측치는 안전하게 'None' 스트링으로 대체하여 파이프라인 인코딩 오류 방지
...
def train_and_cache_pipelines(dataset_key: str, _df, feature_cols, target_col):
    ...
    # 카테고리형 변수와 수치형 변수 자동 식별
    cat_cols = [c for c in feature_cols if _df[c].dtype == 'object']
    num_cols = [c for c in feature_cols if c not in cat_cols]
    
    # 🌟 수동 변환 기능을 완벽히 대체하는 ColumnTransformer 파이프라인 설계
    ...
    # 1. 🎯 Baseline 모델 (기준선 설정을 위한 Dummy Regressor 생성)
